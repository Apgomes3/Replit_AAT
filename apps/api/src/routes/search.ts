...
router.get('/', authenticate, async (req: AuthRequest, res: Response) => {
  ...
  const [projects, systems, equipment, products, materials, documents] = await Promise.all([
    ...
    query(`SELECT 'product' as type, id::text, product_code as code, product_name as name, standard_status as status FROM product_masters WHERE product_name ILIKE $1 OR product_code ILIKE $1 LIMIT 10`, [term]),
