          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                ...
                <th className="text-left px-4 py-2.5 text-xs font-medium text-slate-500 uppercase tracking-wide w-20">Type</th>
                <th className="w-24" />
              </tr>
            </thead>
            <tbody>
              ...
              {!isLoading && categories.length === 0 && (
                <tr><td colSpan={6} className="text-center py-12 text-slate-400">No categories yet</td></tr>
              )}
              {categories.map(cat => (
                <tr key={cat.id} className="border-b border-slate-100 last:border-0">
                  {editId === cat.id ? (
                    <>
                      ...
                      <td className="px-3 py-2"><input value={editForm.code} onChange={e => setEditForm(f => ({ ...f, code: e.target.value.toUpperCase() }))} className={inputCls} /></td>
                      ...
                      <td className="px-3 py-2">
                        <div className="flex items-center gap-1.5">
                          <button onClick={() => handleEdit(cat.id)} disabled={saving} className="p-1.5 text-green-600 hover:bg-green-50 rounded"><Check className="w-4 h-4" /></button>
                    ...
                    <>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1.5">
                          <button onClick={() => startEdit(cat)} className="p-1.5 text-slate-400 hover:text-[#3E5C76] hover:bg-slate-50 rounded"><Pencil className="w-3.5 h-3.5" /></button>
                          {!cat.is_system && (
                            <button onClick={() => handleDelete(cat)} className="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded"><Trash2 className="w-3.5 h-3.5" /></button>
                          )}
                        </div>
                      </td>
                    </>
